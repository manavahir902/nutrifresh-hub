-- Safe fix for notifications and meal plans system
-- This migration safely creates missing tables and policies without conflicts
...
-- 3. Create student_meals table (only if it doesn't exist)
...
-- 4. Create personalized_meal_plan_items table (only if it doesn't exist)
...
-- 5. Enable RLS on new tables (only if not already enabled)
DO $$ 
BEGIN
    IF NOT EXISTS (
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_class WHERE relname = 'student_meals' AND relrowsecurity = true
    ) THEN
...
-- 6. Create RLS policies for messages (only if they don't exist)
DO $$ 
BEGIN
...
-- 7. Create RLS policies for AI suggestions (only if they don't exist)
DO $$ 
BEGIN
    -- Drop existing policies if they exist
...
-- 8. Create RLS policies for student_meals (only if they don't exist)
DO $$ 
BEGIN
    -- Drop existing policies if they exist
...
-- 9. Create RLS policies for personalized_meal_plan_items (only if they don't exist)
DO $$ 
BEGIN
    -- Drop existing policies if they exist
...
-- 11. Insert sample AI suggestions (only if table is empty)
...
-- 12. Update food_items table to include the new categories (only if needed)
