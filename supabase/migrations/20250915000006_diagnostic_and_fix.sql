-- Diagnostic and fix migration for notifications and meal plans
-- This migration will check existing tables and fix them step by step

-- Step 1: Check if personalized_meal_plan_items table exists and its structure
DO $$ 
BEGIN
    -- Drop the table if it exists with wrong structure
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'personalized_meal_plan_items') THEN
        -- Check if it has the correct columns
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'personalized_meal_plan_items' 
            AND column_name = 'user_id'
        ) THEN
            DROP TABLE public.personalized_meal_plan_items CASCADE;
        END IF;
    END IF;
END $$;
...
-- Step 3: Create AI suggestions table (only if it doesn't exist)
...
-- Step 4: Create personalized_meal_plan_items table (fresh creation)
...
-- Step 8: Create RLS policies for AI suggestions
...
-- Step 9: Create RLS policies for personalized_meal_plan_items
...
-- Step 11: Insert sample AI suggestions (only if table is empty)
...
-- Step 12: Update food_items categories
